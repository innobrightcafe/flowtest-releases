...
object SmtpEmailService {
    ...
    suspend fun sendVerificationCode(
        ...
        val rawSmtpPass = listOfNotNull(
            customSmtpPass?.takeIf { isRealSecret(it) },
        ...
        // Use custom user only if custom pass was also supplied, otherwise pair with the configured system app password
        val smtpUser = listOfNotNull(
            if (customSmtpPass != null && isRealSecret(customSmtpPass)) customSmtpUser else null,
     ...
     */
    fun sendViaBackendRelay(to: String, code: String): EmailDispatchResult {
        val targets = (CloudRunApiClient.getBaseUrls() + listOf("http://10.0.2.2:3001")).distinct()
        for (baseUrl in targets) {
            try {
                ...
                val request = Request.Builder()
                    .url("$cleanBase/api/auth/send-code")
                    .post(json.toString().toRequestBody(JSON_TYPE))
                    .build()
                ...
                if (response.isSuccessful || body.contains("\"success\":true")) {
                    return EmailDispatchResult(
                        success = true,
                        methodUsed = "Relay Server ($cleanBase)",
        ...
        return EmailDispatchResult(false, "Relay Server", "Relay offline or unreachable", "All backend relays failed")
    }
}
