...
object CloudRunApiClient {
    ...
    /**
     * Registers the user's funding bank account on Cloud Run backend for automatic webhook matching.
     */
    suspend fun registerFundingAccount(
        ...
        val result = executePost("/api/user/funding-account", json)
    ...
    /**
     * Registers user sender bank account details with backend for automated webhook matching.
     */
    suspend fun saveUserFundingBankAccount(
        identifier: String,
        bankName: String,
        accountNumber: String,
        accountName: String,
        ...
        val hosts = getBaseUrls(customUrl)
        val json = JSONObject().apply {
        ...
        for (host in hosts) {
            ...
            for (path in listOf("/api/v1/user/funding-bank-account", "/api/user/funding-bank-account")) {
                try {
                    val req = Request.Builder()
                        .url("$cleanHost$path")
                        .post(body)
