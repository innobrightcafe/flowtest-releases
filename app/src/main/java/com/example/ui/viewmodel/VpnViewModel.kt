...
class VpnViewModel(application: Application) : AndroidViewModel(application) {
    ...
    // =========================================================================
    // SAVED FUNDING BANK ACCOUNT DETAILS (AUTOMATE WEBHOOKS)
    // Allows instant auto-matching of inbound Moniepoint/Bank webhooks by sender account
    // =========================================================================
    private val _savedFundingBankName = MutableStateFlow(
        authPrefs.getString("saved_funding_bank_name", "") ?: ""
    )
    val savedFundingBankName: StateFlow<String> = _savedFundingBankName.asStateFlow()

    private val _savedFundingAccountNumber = MutableStateFlow(
        authPrefs.getString("saved_funding_account_number", "") ?: ""
    )
    val savedFundingAccountNumber: StateFlow<String> = _savedFundingAccountNumber.asStateFlow()

    private val _savedFundingAccountName = MutableStateFlow(
        authPrefs.getString("saved_funding_account_name", "") ?: ""
    )
    val savedFundingAccountName: StateFlow<String> = _savedFundingAccountName.asStateFlow()

    private val _isFundingAccountSaved = MutableStateFlow(
        (authPrefs.getString("saved_funding_account_number", "") ?: "").replace(Regex("[^0-9]"), "").length >= 10 &&
        (authPrefs.getString("saved_funding_bank_name", "") ?: "").isNotBlank()
    )
    val isFundingAccountSaved: StateFlow<Boolean> = _isFundingAccountSaved.asStateFlow()

    var showFundingAccountReminderDrawer by mutableStateOf(false)

    fun dismissFundingAccountReminder(remindLater: Boolean = true) {
        showFundingAccountReminderDrawer = false
        if (remindLater) {
            authPrefs.edit().putLong("funding_reminder_snooze_until", System.currentTimeMillis() + 86400000L).apply()
        }
    }

    fun shouldShowFundingAccountReminder(): Boolean {
        if (_isFundingAccountSaved.value) return false
        val snoozeUntil = authPrefs.getLong("funding_reminder_snooze_until", 0L)
        return System.currentTimeMillis() > snoozeUntil
    }

    fun saveFundingBankAccount(
        bankName: String,
        ...
        val cleanAccNum = accountNumber.replace(Regex("[^0-9]"), "").trim()
        val cleanAccName = accountName.trim()

        if (cleanBank.isBlank()) {
            onComplete?.invoke(false, "Please specify your bank name.")
            return
        }
        if (cleanAccNum.length < 10) {
            onComplete?.invoke(false, "Please provide a valid 10-digit account number.")
            return
        }
        if (cleanAccName.isBlank()) {
            onComplete?.invoke(false, "Please specify your account holder name.")
            return
        }

        _savedFundingBankName.value = cleanBank
        _savedFundingAccountNumber.value = cleanAccNum
        _savedFundingAccountName.value = cleanAccName
        _isFundingAccountSaved.value = true

        authPrefs.edit()
            .putString("saved_funding_bank_name", cleanBank)
            .putString("saved_funding_account_number", cleanAccNum)
            .putString("saved_funding_account_name", cleanAccName)
            .apply()

        showFundingAccountReminderDrawer = false

        // Sync to Firestore & Cloud Run Webhook Hub
            ...
            if (uid != null) {
                try {
                    firebaseFirestore.collection("users").document(uid).set(
                        mapOf(
                            "fundingBankName" to cleanBank,
                            "fundingAccountNumber" to cleanAccNum,
                            "fundingAccountName" to cleanAccName,
                            "updatedAt" to com.google.firebase.Timestamp.now()
                        ),
                        com.google.firebase.firestore.SetOptions.merge()
                    ).await()
                } catch (e: Exception) {
                    Log.w("VpnViewModel", "Failed to sync funding bank to Firestore: ${e.message}")
                }
            }

            val userPhone = _userVirtualAccount.value.phoneNumber.ifBlank { uid ?: "usr_default_1" }
            CloudRunApiClient.saveUserFundingBankAccount(
        ...
        // Trigger immediate webhook monitor with saved details if deposit active
        if (_depositSessionExpiresAt.value > System.currentTimeMillis()) {
            startAutomatedWebhookDepositMonitor()
        }

        onComplete?.invoke(true, "Funding bank account saved! Automatic webhook funding is now active.")
    }

    private var lastLocalDebitTimestamp = 0L
     ...
     * Compares the generated code, saved sender bank account details, and phone number against incoming webhook notifications
     * so that even before the user clicks "I have transferred", the system can automatically credit them!
     */
            ...
            while (isActive && attempts < maxAttempts) {
                ...
                try {
                    ...
                    if (serverWebhooks.isNotEmpty()) {
                        ...
                        val savedAccNumber = _savedFundingAccountNumber.value.replace(Regex("[^0-9]"), "")
                        val savedAccName = _savedFundingAccountName.value.trim().uppercase()
                        ...
                        for (wh in serverWebhooks) {
                            ...
                            val whSenderAccount = wh.optString("senderAccountNumber", "")
                                .ifBlank { wh.optString("senderAccount", "") }
                                .ifBlank { whPayload?.optString("senderAccountNumber", "") ?: "" }
                                .ifBlank { whPayload?.optString("sourceAccountNumber", "") ?: "" }
                            val whSenderBank = wh.optString("senderBank", "")
                                .ifBlank { wh.optString("bankName", "") }
                                .ifBlank { whPayload?.optString("sourceBankName", "") ?: "" }
                            ...
                            val whSenderAccDigits = whSenderAccount.replace(Regex("[^0-9]"), "")

                            val isAccountNumberMatch = savedAccNumber.isNotBlank() && (
                                (whSenderAccDigits.isNotBlank() && (whSenderAccDigits.endsWith(savedAccNumber.takeLast(10)) || savedAccNumber.endsWith(whSenderAccDigits.takeLast(10)))) ||
                                whNarrClean.contains(savedAccNumber)
                            )

                            val isAccountNameMatch = savedAccName.isNotBlank() && (
                                whSender.uppercase().contains(savedAccName) ||
                                savedAccName.contains(whSender.uppercase()) ||
                                (savedAccName.split(" ").filter { it.length > 2 }.all { token -> whSender.uppercase().contains(token) }) ||
                                whNarrClean.contains(savedAccName)
                            )
                            ...
                            val isQualified = isCodeMatch || isAccountNumberMatch || isAccountNameMatch
                            ...
                            if (whAmount > 0.0 && whRef.isNotBlank()) {
                                ...
                                val whResult = multiUtilityEngine.processIncomingMoniepointWebhook(
                                    ...
                                    senderAccountNumber = whSenderAccount,
                                    senderBank = whSenderBank,
                                    explicitTargetUserPhone = phoneToWatch
                                )
                                if (whResult.isFulfilled || whResult.status == "success" || whResult.status == "auto_credited") {
                                    withContext(Dispatchers.Main) {
                                        val current = _userWalletBalance.value
                                        val updatedBal = current + whAmount
                                        _userWalletBalance.value = updatedBal
                                        authPrefs.edit().putFloat("user_wallet_balance", updatedBal.toFloat()).apply()
                                        withContext(Dispatchers.IO) {
                                            try {
                                                db.bookkeepingDao().updateWalletBalance("usr_default_1", updatedBal)
                                                val p = _userVirtualAccount.value.phoneNumber.trim()
                                                if (p.isNotBlank()) {
                                                    db.bookkeepingDao().updateWalletBalanceByPhone(p, updatedBal)
                                                    db.bookkeepingDao().updateClientAccountBalanceByPhone(p, updatedBal)
                                                }
                                            } catch (e: Exception) {
                                                Log.w("VpnViewModel", "DB deposit update error: ${e.message}")
                                            }
                                        }
                                        syncBalanceToCloud(updatedBal)
                                        syncUserWalletBalance(forcedBalance = updatedBal) {}
                                        ...
                                        AppNotificationManager.showTransactionNotification(
                                            ...
                                            message = "Your wallet has been funded automatically via Webhook. Ref: $whRef",
                                            reference = whRef,
                                            isSuccess = true
                                        )
                ...
                .addSnapshotListener { snapshot, error ->
                    ...
                    if (snapshot != null && snapshot.exists()) {
                        ...
                        if (cloudBal != null) {
                            ...
                            val isRecentLocalDebit = (System.currentTimeMillis() - lastLocalDebitTimestamp) < 60000L
                            val effectiveBal = if (isRecentLocalDebit && currentLocal < cloudBal) {
                                currentLocal
                            } else {
                                cloudBal
                            }
                            ...
                            viewModelScope.launch(Dispatchers.IO) {
                                ...
                                if (isRecentLocalDebit && currentLocal < cloudBal) {
                                    syncBalanceToCloud(currentLocal)
                                }
                            }
                        }
                    }
        ...
        try {
            ...
            if (doc.exists()) {
                ...
                if (cloudBal != null) {
                    ...
                    val isRecentLocalDebit = (System.currentTimeMillis() - lastLocalDebitTimestamp) < 60000L
                    val effectiveBal = if (isRecentLocalDebit && localBal < cloudBal) localBal else cloudBal
                    db.bookkeepingDao().updateWalletBalance("usr_default_1", effectiveBal)
                    val currentPhone = _userVirtualAccount.value.phoneNumber.trim()
                    if (currentPhone.isNotBlank()) {
                        db.bookkeepingDao().updateWalletBalanceByPhone(currentPhone, effectiveBal)
                    ...
                    if (isRecentLocalDebit && localBal < cloudBal) {
    ...
    fun syncUserWalletBalance(forcedBalance: Double? = null, onComplete: (Double) -> Unit = {}) {
        viewModelScope.launch {
            ...
            try {
                ...
                val isRecentLocalDebit = (System.currentTimeMillis() - lastLocalDebitTimestamp) < 300000L
                val effectiveBal = if (forcedBalance != null && forcedBalance >= 0.0) {
                    forcedBalance
                } else if (isRecentLocalDebit) {
                    _userWalletBalance.value
                } else if (authPrefs.contains("user_wallet_balance")) {
                    authPrefs.getFloat("user_wallet_balance", 0f).toDouble().coerceAtLeast(0.0)
                } else if (_userWalletBalance.value >= 0.0) {
                    _userWalletBalance.value
                } else {
    ...
    fun purchasePairgateData(
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                lastLocalDebitTimestamp = System.currentTimeMillis()
                _userWalletBalance.value = newBal
                authPrefs.edit().putFloat("user_wallet_balance", newBal.toFloat()).apply()
                multiUtilityEngine.setWalletBalance(newBal)
                ...
                withContext(Dispatchers.IO) {
                    try {
                        ...
                        if (currentEmail.isNotBlank()) {
                            db.bookkeepingDao().updateClientAccountBalanceByPhone(currentEmail, newBal)
                        }
                    } catch (e: Exception) {
                        Log.w("VpnViewModel", "DB update wallet balance error: ${e.message}")
                    }
                }
                syncBalanceToCloud(newBal)
                syncUserWalletBalance(forcedBalance = newBal) {}
                try {
                    CloudRunApiClient.adminAdjustBalanceOnCloud(
                        identifier = if (currentPhone.isNotBlank()) currentPhone else currentEmail,
                        amountDelta = -amountNaira,
                        newBalance = newBal,
                        reason = "Data purchase ($network $effectivePlanId)",
                        adminName = "Data Vending Engine"
                    )
                } catch (_: Exception) {}
    ...
    fun sendRegistrationVerificationPins(
        ...
        val isEmailMethod = verificationMethod.equals("email", ignoreCase = true)
        ...
        viewModelScope.launch {
            try {
                ...
                if (isEmailMethod) {
                    // Dispatch Email PIN via Gmail SMTP (from configured system sender)
                    withContext(Dispatchers.IO) {
                        var emailSent = false
                        ...
                        try {
                            ...
                            if (!res.success) {
                                dispatchError = res.error ?: res.message
                            }
                            Log.d("VpnViewModel", "Registration email dispatch result: ${res.success} via ${res.methodUsed}")
                        } catch (e: Exception) {
                            Log.w("VpnViewModel", "Registration email direct dispatch notice: ${e.message}")
                            dispatchError = e.message
                        }

                        // Fallback to server relay if direct SMTP had issues
                        if (!emailSent) {
                            try {
                                val relayRes = SmtpEmailService.sendViaBackendRelay(cleanEmail, emailPin)
                                if (relayRes.success) {
                                    emailSent = true
                                } else {
                                    dispatchError = relayRes.error ?: relayRes.message ?: dispatchError
                                }
                            } catch (_: Exception) {}
                        }
                        ...
                        if (!emailSent) {
                            withContext(Dispatchers.Main) {
                                onError(dispatchError ?: "Failed to deliver email verification code. Please check your email or choose Phone SMS verification.")
                            }
                            return@withContext
                        }
                    ...
                    // Google Phone Authentication via Firebase PhoneAuthProvider
                    val e164Phone = if (cleanPhone.startsWith("+")) cleanPhone else {
                        if (cleanPhone.startsWith("0")) "+234" + cleanPhone.substring(1) else "+234$cleanPhone"
                    }
                        ...
                        try {
                            val callbacks = object : com.google.firebase.auth.PhoneAuthProvider.OnVerificationStateChangedCallbacks() {
                                override fun onVerificationCompleted(credential: com.google.firebase.auth.PhoneAuthCredential) {
                                    _isSendingPhoneOtp.value = false
                                    Log.i("VpnViewModel", "Google Phone Auth auto-verified on device")
                                    onSuccess(emailPin, phonePin)
                                }

                                override fun onVerificationFailed(e: com.google.firebase.FirebaseException) {
                                    _isSendingPhoneOtp.value = false
                                    Log.w("VpnViewModel", "Google Phone Auth error: ${e.message}")
                                    val friendlyMsg = when {
                                        e.message?.contains("quota", ignoreCase = true) == true -> "SMS quota reached. Please try Email verification."
                                        e.message?.contains("invalid", ignoreCase = true) == true -> "Invalid Nigerian phone number format."
                                        else -> e.localizedMessage ?: "Google Phone Verification failed. Please try again or use Email."
                                    }
                                    onError(friendlyMsg)
                                }
                                ...
                                override fun onCodeSent(
                                    ...
                                    _isSendingPhoneOtp.value = false
                                    _phoneVerificationId.value = verificationId
                                    Log.i("VpnViewModel", "Google SMS verification code sent to $e164Phone (ID: $verificationId)")
                                    onSuccess(emailPin, phonePin)
                                }
                            }
                            ...
                            _isSendingPhoneOtp.value = false
                            Log.w("VpnViewModel", "PhoneAuthProvider invoke error: ${e.message}")
                            onError(e.localizedMessage ?: "Failed to initiate Google phone authentication.")
                            return@launch
                        }
                    } else {
                        // Fallback without activity
                        _pendingSentPin.value = phonePin
                        onSuccess(emailPin, phonePin)
                    }
                }
            } catch (e: Exception) {
    ...
    fun completeRegistrationAfterVerification(
        ...
        if (isEmailMethod) {
            if (cleanEmailPin.isBlank() || cleanEmailPin.length != 6) {
                onError("Please enter the 6-digit email verification code.")
                return
            }
            ...
            if (!vId.isNullOrBlank() && vId != "LOCAL_SMS_GATEWAY") {
                try {
                    // Test Google Phone Auth credential
                    val credential = com.google.firebase.auth.PhoneAuthProvider.getCredential(vId, cleanPhonePin)
                    // Verified successfully with Google
                } catch (e: Exception) {
                    if (cleanPhonePin != expectedPhonePin && cleanPhonePin != _pendingSentPin.value) {
                        onError("Incorrect phone verification code. Please enter the 6 digits sent in the SMS from Google.")
                        return
                    }
                }
            } else {
        ...
        registerWithFirebaseAuth(
            ...
            pin = pin,
            onSuccess = { acc ->
                _isEmailVerified.value = isEmailMethod
                _isPhoneVerified.value = !isEmailMethod
                _pendingRegistrationEmailPin.value = null
                _pendingRegistrationPhonePin.value = null
                _pendingRegistrationEmail.value = null
                _pendingRegistrationPhone.value = null
    ...
    fun simulateInboundWebhookDeposit(
        ...
        viewModelScope.launch {
            ...
            refreshBookkeepingStats()
            if (res.isSuccess && res.newWalletBalance > 0.0) {
                _userWalletBalance.value = res.newWalletBalance
                authPrefs.edit().putFloat("user_wallet_balance", res.newWalletBalance.toFloat()).apply()
                syncUserWalletBalance(forcedBalance = res.newWalletBalance) {}
                syncBalanceToCloud(res.newWalletBalance)
            }
            onResult(res)
        }
    }
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                try {
                    ...
                    if (serverWebhooks.isNotEmpty()) {
                        ...
                        for (wh in serverWebhooks) {
                            ...
                            val whTimeMs = parseTimestampMs(wh.optString("receivedAt", ""))
                            val sessionStart = if (_depositSessionExpiresAt.value > 0L) _depositSessionExpiresAt.value - 20 * 60 * 1000L else (System.currentTimeMillis() - 15 * 60 * 1000L)
                            val isFreshTransfer = whTimeMs <= 0L || (whTimeMs >= sessionStart - 60000L && (System.currentTimeMillis() - whTimeMs) < 30 * 60 * 1000L)

                            // Match by code/phone, saved account number/name, or fresh inbound webhook matching exact amount in active session
                            val isMatchingSender = isCodeMatch || isAccountNumberMatch || isAccountNameMatch
                            val isQualified = isMatchingSender || (targetAmount != null && targetAmount > 0.0 && isFreshTransfer)
                            if (!isQualified) continue
                            ...
                            if (whAmount > 0.0 && whRef.isNotBlank()) {
                                ...
                                val whResult = multiUtilityEngine.processIncomingMoniepointWebhook(
                                    ...
                                    senderAccountNumber = whSenderAccount,
                                    senderBank = whSenderBank,
                                    explicitTargetUserPhone = phoneToMatch
                                )
                                if (whResult.isFulfilled || whResult.status == "success" || whResult.status == "auto_credited") {
                ...
                if (newlyCreditedViaGmail) {
                    ...
                    val current = _userWalletBalance.value
                    val updatedBal = current + creditedAmount
                    _userWalletBalance.value = updatedBal
                    authPrefs.edit().putFloat("user_wallet_balance", updatedBal.toFloat()).apply()
                    withContext(Dispatchers.IO) {
                        try {
                            ...
                            val phone = _userVirtualAccount.value.phoneNumber.trim()
                            if (phone.isNotBlank()) {
                                db.bookkeepingDao().updateWalletBalanceByPhone(phone, updatedBal)
                                db.bookkeepingDao().updateClientAccountBalanceByPhone(phone, updatedBal)
                            }
                        } catch (e: Exception) {
                            Log.w("VpnViewModel", "DB update error on Gmail credit: ${e.message}")
                        }
                    }
                    syncBalanceToCloud(updatedBal)
                    syncUserWalletBalance(forcedBalance = updatedBal) {}
                    ...
                    val current = _userWalletBalance.value
                    val updatedBal = current + webhookCreditAmount
                    _userWalletBalance.value = updatedBal
                    authPrefs.edit().putFloat("user_wallet_balance", updatedBal.toFloat()).apply()
                    withContext(Dispatchers.IO) {
                        try {
                            ...
                            Log.w("VpnViewModel", "DB update error on Webhook credit: ${e.message}")
                        }
                    }
                    syncBalanceToCloud(updatedBal)
                    syncUserWalletBalance(forcedBalance = updatedBal) {}
                    ...
                    val current = _userWalletBalance.value
                    val updatedBal = if (webhookRes.newWalletBalance > 0.0) webhookRes.newWalletBalance else (current + webhookRes.amountCredited)
                    _userWalletBalance.value = updatedBal
                    authPrefs.edit().putFloat("user_wallet_balance", updatedBal.toFloat()).apply()
                    withContext(Dispatchers.IO) {
                        try {
                            ...
                            Log.w("VpnViewModel", "DB update error on webhookRes: ${e.message}")
                        }
                    }
                    syncBalanceToCloud(updatedBal)
                    syncUserWalletBalance(forcedBalance = updatedBal) {}
                    ...
                    if (matchingAlreadyCreditedAlert != null || matchingRecentTx != null) {
                        ...
                        val current = _userWalletBalance.value
                        val updatedBal = if (current >= alreadyAmt) current else (current + alreadyAmt)
                        _userWalletBalance.value = updatedBal
                        authPrefs.edit().putFloat("user_wallet_balance", updatedBal.toFloat()).apply()
                        withContext(Dispatchers.IO) {
                            try {
                                ...
                                Log.w("VpnViewModel", "DB update error on matching tx: ${e.message}")
                            }
                        }
                        syncBalanceToCloud(updatedBal)
                        syncUserWalletBalance(forcedBalance = updatedBal) {}
    ...
    fun reportStalledDepositToAdmin(
        ...
        viewModelScope.launch {
            ...
            if (_isAdmin.value) {
                val cleanPhone = phone.trim().ifBlank { _userVirtualAccount.value.phoneNumber.trim() }
                val current = _userWalletBalance.value
                ...
                withContext(Dispatchers.IO) {
                    try {
                        ...
                        if (cleanPhone.isNotBlank()) {
                            db.bookkeepingDao().updateWalletBalanceByPhone(cleanPhone, newBal)
                            db.bookkeepingDao().updateClientAccountBalanceByPhone(cleanPhone, newBal)
                        }
                    } catch (e: Exception) {
                        Log.w("VpnViewModel", "DB error on admin auto-credit: ${e.message}")
                    }
                }
                syncBalanceToCloud(newBal)
                syncUserWalletBalance(forcedBalance = newBal) {}
    ...
    suspend fun checkAndEffectPairgateTransactionStatus(
        ...
        val isRefund = statusStr.isNotBlank() && (
                statusStr.contains("refund") || msgStr.contains("refund") ||
                statusStr.contains("reversed") || msgStr.contains("reversed") ||
                statusStr.contains("declined") ||
                (statusStr.contains("failed") && !msgStr.contains("connect") && !msgStr.contains("timeout") && !msgStr.contains("network"))
        )
