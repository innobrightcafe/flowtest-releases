...
const GMAIL_USER = process.env.GMAIL_USER || process.env.SMTP_EMAIL_ADDRESS || 'help.inkwall@gmail.com';
const GMAIL_APP_PASSWORD = (process.env.GMAIL_APP_PASSWORD || process.env.SMTP_EMAIL_APP_PASSWORD || 'khzyordukanjnaek').replace(/\s+/g, '');
...
app.post(MONIEPOINT_WEBHOOK_PATHS, (req, res) => {
  ...
  const senderAccountNumber = payload.sourceAccountNumber || 
                              payload.senderAccountNumber || 
                              payload.senderAccount || 
                              payload.sourceAccount ||
                              payload.payerAccountNumber ||
                              payload.payerAccount ||
                              payload.data?.sourceAccountNumber || 
                              payload.data?.senderAccountNumber || 
                              payload.data?.senderAccount || 
                              payload.data?.sourceAccount ||
                              payload.eventData?.sourceAccountNumber || 
                              payload.transaction?.sourceAccountNumber || 
                              '';

  const senderBank = payload.sourceBankName || 
                     payload.senderBank || 
                     payload.sourceBank || 
                     payload.bankName || 
                     payload.payerBank || 
                     payload.data?.sourceBankName || 
                     payload.data?.senderBank || 
                     payload.data?.sourceBank || 
...
// ==========================================
// USER FUNDING BANK ACCOUNTS REGISTRY
// Maps user identifier to sender bank details for instant webhook matching
// ==========================================
const userFundingBankAccounts = new Map();

app.post(['/api/v1/user/funding-bank-account', '/api/user/funding-bank-account'], (req, res) => {
  const { identifier, bankName, accountNumber, accountName } = req.body || {};
  if (!identifier || !accountNumber) {
    return res.status(400).json({ status: 'error', message: 'identifier and accountNumber are required' });
  }
  const cleanId = String(identifier).trim().toLowerCase();
  const cleanAccNum = String(accountNumber).replace(/[^0-9]/g, '');
  const data = {
    identifier: cleanId,
  ...
  userFundingBankAccounts.set(cleanId, data);
  if (cleanAccNum.length >= 10) {
    userFundingBankAccounts.set(cleanAccNum.slice(-10), data);
  }
  console.log(`[FUNDING ACCOUNT REGISTERED] User ${cleanId} linked sender bank: ${data.bankName} - ${data.accountNumber} (${data.accountName})`);
  res.status(200).json({ status: 'success', message: 'Funding bank account registered successfully', data });
});

app.get(['/api/v1/user/funding-bank-account/:identifier', '/api/user/funding-bank-account/:identifier'], (req, res) => {
  const id = String(req.params.identifier || '').trim().toLowerCase();
  const cleanNum = id.replace(/[^0-9]/g, '');
  const data = userFundingBankAccounts.get(id) || (cleanNum.length >= 10 ? userFundingBankAccounts.get(cleanNum.slice(-10)) : null);
  if (!data) {
    return res.status(404).json({ status: 'error', message: 'Funding account not found' });
  }
